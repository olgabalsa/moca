...
def save_arrays(mocca_rvs, mocca_ervs, mocca_jds, dir, star):
    tn = Time.now().isot[:16]
    full_dir = f'{dir}/mocca/{star}'
    os.makedirs(full_dir, exist_ok = True)
    
    np.save(f'{full_dir}/mocca_rvs_{tn}', mocca_rvs)
    np.save(f'{full_dir}/mocca_ervs_{tn}', mocca_ervs)
    np.save(f'{full_dir}/mocca_jds_{tn}', mocca_jds)
...
def plot_RV_jd(jd, num_pl, rv, erv, K, per, t0, c, d, alpha, dir, star):
    ...
    plt.savefig(f'{full_dir}/plot_moccaRVs_{tn}.pdf', dpi = 300, bbox_inches = 'tight')
